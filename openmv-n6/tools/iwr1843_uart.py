...
TLV_STATS = 6                     # 6 x uint32 timing/load counters

# timeCpuCycles is a free-running counter latched by the DSP when the frame is
# emitted, so it dates the MEASUREMENT rather than its arrival on the host --
# it is immune to USB scheduling, kernel buffering and read() timing, all of
# which sit between the radar and any host timestamp. It wraps at 2**32; use
# frame_clock.Unwrapper(period=CYCLES_PERIOD) to get a continuous axis.
#
# The tick rate is NOT assumed anywhere. Regress unwrapped cycles against
# frame_no over a minute and the slope is the rate, in cycles per frame period.
CYCLES_PERIOD = 1 << 32

# Largest frame any enabled TLV set can produce, used to sanity-check the header's
# totalPacketLen. Bounding it by the BUFFER instead is what makes a corrupt length
# catastrophic: at 65536 a single bit-flip makes feed() wait for 65 kB, which at
...
class RadarReader:
    ...
    def __init__(self, max_buffer=65536, max_frame=MAX_FRAME_BYTES):
        self._buf = b""
        self._max = max_buffer
        self._max_frame = max_frame
        # A resync is not an error worth raising -- the next frame is usually
        # fine -- but it must not be silent either, or a flaky cable reads as a
        # quiet link. Callers that care can watch this climb.
        self.resyncs = 0
    ...
    def feed(self, data):
        ...
        {'frame': int, 'n_obj': int, 'points': [(x_fwd, y_left, z_up, v), ...],
         ...
         'cycles': int, 'stats': dict or None,
         'batch_i': int, 'batch_n': int, 'bytes_after': int}
        ...
        batch_i/batch_n/bytes_after exist because one read() can yield several
        frames, which destroys per-frame arrival timing unless the caller can
        undo it. bytes_after is how many bytes of THIS read still followed the
        frame, so a caller that stamps arrival once per read can recover each
        frame's own arrival as t_arr - bytes_after / (baud / 10).
        """
        if data:
            self._buf += data
        ...
        while True:
            ...
            if total_len < HDR_LEN or total_len > self._max_frame:
            ...
            fr = self._parse_tlvs(packet, n_obj, n_tlv, frame_no, cpu_cycles)
            if fr is not None:
                fr["bytes_after"] = len(self._buf)
                frames.append(fr)
        for i, fr in enumerate(frames):
            fr["batch_i"], fr["batch_n"] = i, len(frames)
        return frames

    def _parse_tlvs(self, pkt, n_obj, n_tlv, frame_no, cpu_cycles=0):
        points, snr, noise, stats = [], None, None, None
        off = HDR_LEN
        for _ in range(n_tlv):
            ...
            if tlv_type == TLV_DETECTED_POINTS:
                ...
                for i in range(n):
                    ...
                    snr.append(s * 0.1)
                    noise.append(nz * 0.1)
            elif tlv_type == TLV_STATS and tlv_len >= 24:
                # Already enabled by `guiMonitor -1 1 0 0 0 0 1` and previously
                # skipped. interframe_margin_us is the headroom the DSP has left
                # -- the number that says whether a shorter frame period will
                # overrun, instead of finding out from dropped frame numbers.
                (ifp, tout, ifm, tmargin, load, tload) = struct.unpack_from(
                    "<6I", pkt, off)
                stats = {"interframe_proc_us": ifp, "transmit_out_us": tout,
                         "interframe_margin_us": ifm, "transmit_margin_us": tmargin,
                         "interframe_cpu_load_pct": load,
                         "active_cpu_load_pct": tload}
            off += tlv_len
